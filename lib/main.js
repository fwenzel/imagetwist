...
const boxnotify = require('boxnotify');
...
var menu = cm.Menu({
    ...
    onMessage: function(msg) {
        ...
        var pagemod = tabs.activeTab.attach({
            contentScriptFile: [data.url('js/image_utils.js'),
        ...
        worker.port.on('cancel', function() {  // No rotation performed.
            boxnotify.notify('No rotation necessary or no EXIF rotation ' +
                             'data found!');
...
// "Revert" for once-rotated images.
cm.Item({
    context: cm.SelectorContext('img[data-imagetwist-origsrc]'),
    contentScriptFile: [data.url('js/contextmenu.js'),
                        data.url('js/image_utils.js')],
    label: 'Restore original image',
    data: 'revert',
    image: data.url('img/undo.png')
});
